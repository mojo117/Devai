/**
 * Firecrawl Tools - Web research and stock data via Firecrawl API.
 */
...
// ── Stock data types (inline — previously in types/stock.js) ──
...
// ── Web research functions (used by CHAPO via tool handlers) ──
...
// ── Stock data functions (Firecrawl Agent API) ──
...
Return: symbol, name, peRatio (KGV), psRatio (KUV), dividendYield (percent), price (EUR). Use null if not available.`;
  ...
  try {
    const startRes = await fetch(`${FIRECRAWL_BASE_URL}/v2/agent`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json'
    ...
    while (Date.now() - startTime < 60000) {
      ...
      const statusRes = await fetch(`${FIRECRAWL_BASE_URL}/v2/agent/${id}`, {
