...
### Test Criteria Matrix (Loop QA)

| ID | Area | Scenario | Expected |
|----|------|----------|----------|
| LC-ASK-01 | ASK | User: "Mach das besser" (ambiguous) | CHAPO triggers `askUser`, returns `waiting_for_user`, pending question is persisted in state |
| LC-ASK-02 | ASK Resume | User replies to queued question | `handleUserResponse()` resumes flow and final answer references provided clarification |
| LC-ANS-01 | ANSWER | User: "Hallo, wie geht's?" | Direct response without unnecessary tool/delegation calls |
| LC-ANS-02 | ANSWER Validation | Non-trivial question with direct answer | Response delivered and validation event logged (advisory only, no blocking) |
| LC-TOL-01 | TOOL | User: "Zeig mir package.json" | CHAPO executes `fs_readFile` (or equivalent), tool result is fed back, final answer includes file insight |
| LC-TOL-02 | TOOL Error Recovery | Force invalid path/tool input | Error is fed back into loop context; conversation continues (no hard crash) |
| LC-AGT-01 | AGENTS DEVO | User asks for code change | CHAPO delegates to DEVO and returns synthesized final answer |
| LC-AGT-02 | AGENTS SCOUT | User asks for web/codebase research | CHAPO/DEVO delegates to SCOUT and uses structured scout result in final answer |
| LC-AGT-03 | Plan Task Execution | Approve plan with 2+ tasks | `executePlan()` runs tasks via `executePlanTaskWithLoop()`, task states update (`in_progress` → `completed`/`failed`) |
| LC-CTX-01 | Gate/State | Bare "yes/no" with pending approval/question | Correct gate handler runs (`handleUserApproval`/`handleUserResponse`) instead of new request flow |
